...
public class client extends javax.swing.JFrame {
    ...
    public client() {
        ...
        System.out.print("Enter your name : ");     // client name
        name = sc.next();
        initComponents();
        sc.close();
    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try {  
            ...
            jTextArea1.setText(jTextArea1.getText() + "\n Me:" + msgout);       // sets the text of textArea
            Output.writeUTF(name + ":" + msgout);           // adds the message
            jTextField1.setText("");            // clears the textField for new message
       } catch (Exception e) {}  
    ...
    public static void main(String args[]) {
        ...
        try {
            S = new Socket("127.0.0.1", 3000);          // sends a request to the server to connect to port no - 3000
            Input = new DataInputStream(S.getInputStream());  
            Output = new DataOutputStream(S.getOutputStream());  
            String msgin = "";
            while (!msgin.equals("exit")) {  
                msgin = Input.readUTF();            // reads the message from server
                jTextArea1.setText(jTextArea1.getText() + "\n " + msgin);   // sets the textArea with updated message
            }
